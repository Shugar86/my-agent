...
```bash
cp .env.example .env
# OPENROUTER_API_KEY=sk-or-...  (primary LLM; demo OK без ключей)
...
- 2 GB RAM minimum
- `OPENROUTER_API_KEY` для live-чата и реального demo (без ключей — mock)
- `KIMI_API_KEY` опционально для Kimi-профилей
...
| `TAVILY_API_KEY` | Веб-поиск (рекомендуется для demo) |
