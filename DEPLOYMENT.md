...
```bash
cp .env.example .env
# OPENROUTER_API_KEY=sk-or-v1-...  (live agent preview + chat)
...
Public demo работает **без API-ключей** — mock fallback на agent preview.
...
- 2 GB RAM minimum
- `OPENROUTER_API_KEY` (опционально — без ключа demo показывает mock)
...
| `OPENROUTER_API_KEY` | Primary LLM (agent preview, chat, workflows) |
| `AGENT_SECRET_KEY` | Random 32+ chars |
| `AGENT_PASSWORD` | ≥ 12 chars, change from default |
...
| Agent preview 503 | Нет `OPENROUTER_API_KEY` — UI fallback; для live LLM добавьте ключ |
