...
```bash
cp .env.example .env
# OPENROUTER_API_KEY=sk-or-v1-...  (primary LLM; demo mock без ключей)
# TAVILY_API_KEY=tvly-...           (live web search в demo)
...
- 2 GB RAM minimum
- `OPENROUTER_API_KEY` (primary LLM; demo работает без ключей — mock)
- `TAVILY_API_KEY` (опционально — live web search)
...
| `KIMI_API_KEY` | Опционально (legacy per-agent) |
