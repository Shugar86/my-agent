...
```bash
cp .env.example .env
# OPENROUTER_API_KEY=sk-or-v1-...  (primary LLM; demo OK без ключей)
...
- 2 GB RAM minimum
- `OPENROUTER_API_KEY` (primary; demo работает без ключей — mock)
...
| `TAVILY_API_KEY` | Web search в live demo (опционально) |
