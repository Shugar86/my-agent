...
```bash
cp .env.example .env
# OPENROUTER_API_KEY=sk-or-v1-...  (опционально; demo работает без ключей)
...
- 2 GB RAM minimum
- `OPENROUTER_API_KEY` (+ `TAVILY_API_KEY` для live search; demo работает без ключей — mock)
...
| `OPENROUTER_API_KEY` | Primary LLM (профиль `balanced`) |
| `TAVILY_API_KEY` | Веб-поиск для live demo |
