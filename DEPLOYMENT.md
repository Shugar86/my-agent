...
```bash
cp .env.example .env
# OPENROUTER_API_KEY=sk-or-v1-...  (primary LLM; опционально KIMI_API_KEY)
...
- 2 GB RAM minimum
- `OPENROUTER_API_KEY` (live chat/demo); `KIMI_API_KEY` опционально; без ключей — mock demo
...
| `KIMI_API_KEY` | Опционально (Kimi / legacy profiles) |
