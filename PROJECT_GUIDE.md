...
| Fallback | Gemini через OpenRouter или Kimi (`KIMI_API_KEY`) |
...
Профили моделей: `core/configurator.py` (`MODEL_PROFILES`) + `config/agent.json`. Файл `config/models.yaml` — устаревший справочник, runtime его не читает.
...
| URL | Описание |
|-----|----------|
| `/showcase#playground` | **Канонический demo** — Competitor Intelligence 90s → DOCX (без login) |
...
Редиректы: `/app/agents`, `/app/knowledge`, `/app/mcp` → `settings?tab=…`; `/welcome` → `/`.
...
| POST | `/api/demo/public/run` | Public investor demo (без auth) |
