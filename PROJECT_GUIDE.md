...
**My Agent** — AI Agent OS: multi-agent чат, визуальный workflow builder и маркетплейс шаблонов. Публичный agent preview на лендинге работает без регистрации.

| Компонент | Значение |
|-----------|----------|
| Primary LLM | OpenRouter (`OPENROUTER_API_KEY`) |
| Fallback | Free-модели через `core/configurator.py` |
...
```bash
cp .env.example .env
# OPENROUTER_API_KEY — для live agent preview (без ключа — mock fallback)
...
| POST | `/api/demo/public/agent-preview` | Live agent preview (public) |
| POST | `/api/demo/public/agent-chat` | Follow-up chat с preview |
...
10 профилей в `agents/registry.json`: **universal**, researcher, developer, marketer, data_analyst, slides, docs, media, data_engineer, news.
