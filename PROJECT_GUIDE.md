...
| Fallback | `google/gemini-2.5-flash-preview` через тот же OpenRouter key |
| Legacy | Kimi Code API (`KIMI_API_KEY`) — per-agent overrides в `agents/registry.json` |
...
| Симптом | Решение |
|---------|---------|
| 500 в чате | Проверить `OPENROUTER_API_KEY`, перезапустить сервер; в Docker — см. [TROUBLESHOOTING.md](TROUBLESHOOTING.md) § Postgres sessions |
