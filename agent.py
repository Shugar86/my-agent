...
║                                                              ║
║  1. API ключи (NeuroAPI или OpenRouter)                      ║
...
def cmd_setup(args):
    ...
    print("\nДоступные модели:")
    print("  1. fast      — NeuroAPI gpt-5.4-nano (~1.5с)")
    print("  2. balanced  — OpenRouter owl-alpha (~6с)")
    print("  3. smart     — Claude Sonnet (~8с)")
    print("  4. local     — Ollama на localhost")
    
    choice = input("\nВыберите модель [1]: ").strip() or "1"
    profiles = {
        "1": ("openai/gpt-5.4-nano", "https://neuroapi.host/v1", "NEUROAPI_API_KEY"),
        "2": ("openrouter/owl-alpha", "https://openrouter.ai/api/v1", "OPENROUTER_API_KEY"),
