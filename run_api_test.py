...
neuro_key = _require_env("NEUROAPI_API_KEY")
openrouter_key = _require_env("OPENROUTER_API_KEY")

# Test 1: NeuroAPI simple
results["neuroapi_simple"] = test_provider(
    "NeuroAPI",
    "openai/gpt-5.4-nano",
    neuro_key,
...
results["neuroapi_tools"] = test_provider(
    "NeuroAPI+tools",
    "openai/gpt-5.4-nano",
    neuro_key,
    "https://neuroapi.host/v1",
    [{"role": "user", "content": "search for AI news"}],
    tools[:3] if tools else None,
)

# Test 3: OpenRouter simple
results["openrouter_simple"] = test_provider(
    "OpenRouter",
...
results["openrouter_tools"] = test_provider(
    "OpenRouter+tools",
    "openrouter/owl-alpha",
    openrouter_key,
    "https://openrouter.ai/api/v1",
...
try:
    ...
    results["tavily"] = {
        "ok": True,
        "time": time.time() - t0,
        "results": len(r) if isinstance(r, list) else 0,
        "error": None,
    }
except (RuntimeError, ValueError, OSError) as e:
