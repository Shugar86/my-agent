...
**Autonomous Workflow OS** — визуальный конструктор workflow, маркетплейс шаблонов, multi-agent чат и deep research на базе OpenRouter (профиль `balanced` / `openrouter/owl-alpha`).
...
```bash
cp .env.example .env
# Минимум: OPENROUTER_API_KEY (+ TAVILY_API_KEY для live search); demo работает и без ключей
...
|------------|------------|
| `OPENROUTER_API_KEY` | Primary LLM (OpenRouter, профиль `balanced`) |
| `KIMI_API_KEY` | Опционально — профиль `kimi` в CLI / Settings |
| `TAVILY_API_KEY` | Веб-поиск для live demo и research |
...
| Тема | Файл |
|------|------|
| RU-руководство | [PROJECT_GUIDE.md](PROJECT_GUIDE.md) · [WINDOWS_LAUNCH.md](WINDOWS_LAUNCH.md) |
