...
**Autonomous Workflow OS** — визуальный конструктор workflow, маркетплейс шаблонов, multi-agent чат и deep research (OpenRouter / Kimi через `config/agent.json`).
...
```bash
cp .env.example .env
# Для live-чата и реального demo: OPENROUTER_API_KEY (+ TAVILY_API_KEY для поиска)
# Demo и showcase работают и без ключей (mock fallback)
...
| `KIMI_API_KEY` | Опционально — Kimi Code API для отдельных профилей |
