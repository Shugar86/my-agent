...
```bash
cp .env.example .env
# Для live chat: OPENROUTER_API_KEY (+ TAVILY_API_KEY для поиска). Demo работает и без ключей.
...
|------------|------------|
| `OPENROUTER_API_KEY` | Primary LLM (`openrouter/owl-alpha` в `config/agent.json`) |
