...
```bash
cp .env.example .env
# Для live-чата: OPENROUTER_API_KEY (+ TAVILY_API_KEY для веб-поиска)
# Demo работает и без ключей (mock replay)
...
├── config/               # agent.json (LLM defaults)
...
|------------|------------|
| `OPENROUTER_API_KEY` | Primary LLM (OpenRouter, `openrouter/owl-alpha`) |
| `KIMI_API_KEY` | Опционально — профиль `kimi` в `core/configurator.py` |
